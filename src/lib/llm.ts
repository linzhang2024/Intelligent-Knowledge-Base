...
3. **诚实原则**：
   - 如果当前上下文中明确标有"未检索到相关内容"的提示，请直接使用该提示中推荐的回答方式。
   - 其他情况，如果上下文没有足够的信息来回答问题，请诚实地说"根据当前知识库中的内容，我无法回答这个问题"。
...
function buildContext(searchResults: SearchResult[]): string {
  if (searchResults.length === 0) {
    return `【未检索到相关内容】
当前知识库中没有找到与用户问题直接相关的内容（可能是相似度低于阈值 0.38）。

推荐回答方式：
"在现有文档中未匹配到紧密相关的内容，建议您尝试更换关键词提问。"

注意：请直接使用上述推荐回答方式回复用户。`;
  }
